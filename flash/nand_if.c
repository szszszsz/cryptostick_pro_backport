...
#ifdef USE_STM3210E_EVAL // RB not used
/* Includes ------------------------------------------------------------------*/
#include "fsmc_nand.h"
#include "mass_mal.h"
#include "memory.h"
...
static uint16_t NAND_CleanLUT(uint8_t ZoneNbr) {
  ...
  LUT_Item = LUT[BlockIdx] for (BlockIdx == MAX_LOG_BLOCKS_PER_ZONE;
...
static uint16_t NAND_BuildLUT(uint8_t ZoneNbr) {
  ...
  *****************************************************************************/
  ...
  while (pCurrentBlock < MAX_PHY_BLOCKS_PER_ZONE) {
    ...
    SpareArea = ReadSpareArea(pCurrentBlock * NAND_BLOCK_SIZE +
                              (ZoneNbr * NAND_BLOCK_SIZE * MAX_PHY_BLOCKS_PER_ZONE));
