 ...
 *    Warning 528: Symbol 'NVIC_DisableIRQ(unsigned int)' (line 419, file ./cm3_core.h) not
 * referenced \n
 *     \n
 *    --- Error 751: --- \n
 *    } InterruptType_Type; \n
...
#define __INLINE                                                                                   \
  inline /*!< inline keyword for IAR Compiler. Only avaiable in High optimization mode! */
...
static __INLINE void NVIC_SetPriorityGrouping(uint32_t priority_grouping) {
  ...
  reg_value = ((reg_value | NVIC_AIRCR_VECTKEY |
                (priority_grouping << 8))); /* Insert write key and priorty group */
  SCB->AIRCR = reg_value;
}
...
static __INLINE IRQn_Type NVIC_GetPendingIRQ(IRQn_Type IRQn) {
  return ((IRQn_Type)(NVIC->ISPR[(uint32_t)(IRQn) >> 5] &
                      (1 << ((uint32_t)(IRQn)&0x1F)))); /* Return Interrupt bit or 'zero' */
}
...
static __INLINE IRQn_Type NVIC_GetActive(IRQn_Type IRQn) {
  return ((IRQn_Type)(NVIC->IABR[(uint32_t)(IRQn) >> 5] &
                      (1 << ((uint32_t)(IRQn)&0x1F)))); /* Return Interruptnumber or 'zero' */
}
...
#define SYSTICK_TICKINT                                                                            \
  1 /* Config-Bit to enable or disable the SysTick interrupt                 */
#define SYSTICK_CLKSOURCE                                                                          \
  2 /* Clocksource has the offset 2 in SysTick Control and Status Register   */
#define SYSTICK_MAXCOUNT                                                                           \
  ((1 << 24) - 1) /* SysTick MaxCount                                                      */
...
static __INLINE uint32_t ITM_SendChar(uint32_t ch) {
  ...
  if ((CoreDebug->DEMCR & CoreDebug_DEMCR_TRCENA) && (ITM->TCR & ITM_TCR_ITMENA) &&
      (ITM->TER & (1UL << 0))) {
