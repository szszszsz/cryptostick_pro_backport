...
typedef struct {
  ...
  FSMC_NORSRAMTimingInitTypeDef
      *FSMC_ReadWriteTimingStruct; /* Timing Parameters for write and read access if the
...
#define IS_FSMC_GETFLAG_BANK(BANK)                                                                 \
  (((BANK) == FSMC_Bank2_NAND) || ((BANK) == FSMC_Bank3_NAND) || ((BANK) == FSMC_Bank4_PCCARD))
...
#define IS_FSMC_WAIT_POLARITY(POLARITY)                                                            \
  (((POLARITY) == FSMC_WaitSignalPolarity_Low) || ((POLARITY) == FSMC_WaitSignalPolarity_High))
...
#define IS_FSMC_WRITE_OPERATION(OPERATION)                                                         \
  (((OPERATION) == FSMC_WriteOperation_Disable) || ((OPERATION) == FSMC_WriteOperation_Enable))
...
#define IS_FSMC_ECCPAGE_SIZE(SIZE)                                                                 \
  (((SIZE) == FSMC_ECCPageSize_256Bytes) || ((SIZE) == FSMC_ECCPageSize_512Bytes) ||               \
   ((SIZE) == FSMC_ECCPageSize_1024Bytes) || ((SIZE) == FSMC_ECCPageSize_2048Bytes) ||             \
   ((SIZE) == FSMC_ECCPageSize_4096Bytes) || ((SIZE) == FSMC_ECCPageSize_8192Bytes))
...
#define IS_FSMC_GET_IT(IT)                                                                         \
  (((IT) == FSMC_IT_RisingEdge) || ((IT) == FSMC_IT_Level) || ((IT) == FSMC_IT_FallingEdge))
/**
  * @}
  */
