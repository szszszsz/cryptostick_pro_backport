...
const unsigned int Dmul64vsDI[] = {0,   64,   128, 256, 512, 1024, 2048, 0,
                                   768, 1280, 32,  16,  8,   4,    2,    1};
...
unsigned char IFD_ApplyParametersStructure(void) {
  ...
  //	SC_SetHwParams
  //(IccParameters.FiDi,IccParameters.T01ConvChecksum,IccParameters.GuardTime,IccParameters.WaitingInteger);
...
unsigned char IFD_XfrBlock(unsigned char *pBlockBuffer, unsigned int *pBlockSize,
  ...
  // why not this ??? T0 protocol ? >>			ErrorCode = IFD_XfrCharT1(pBlockBuffer,
  // pBlockSize);
...
unsigned char IFD_XfrCharT0(unsigned char *pBlockBuffer, unsigned int *pBlockSize,
...
unsigned char IFD_XfrCharT1(unsigned char *pBlockBuffer, unsigned int *pBlockSize) { return 0xF6; }
...
unsigned char IFD_XfrTpduT0(unsigned char *pBlockBuffer, unsigned int *pBlockSize) { return 0xF6; }
...
unsigned char IFD_XfrTpduT1(unsigned char *pBlockBuffer, unsigned int *pBlockSize) { return 0xF6; }
...
unsigned char IFD_SetParameters(unsigned char *pParamBuffer, unsigned char T01) {
  ...
  if ((T01 == 0x00) && (NewIccParameters.T01ConvChecksum != 0x00) &&
      (NewIccParameters.T01ConvChecksum != 0x02)) {
    return SLOTERROR_BAD_T01CONVCHECKSUM;
  }

  if ((T01 == 0x01) && (NewIccParameters.T01ConvChecksum != 0x10) &&
      (NewIccParameters.T01ConvChecksum != 0x11) && (NewIccParameters.T01ConvChecksum != 0x12) &&
