...
void GenerateChainedTPDU(typeSmartcardTransfer *tSCT) {
  tSCT->cTPDU[CCID_TPDU_NAD] = 0; // Node Address (NAD)
  tSCT->cTPDU[CCID_TPDU_PCD] =
      ((tSCT->cTPDUSequence & 1) << 4) + CCID_TPDU_R_BLOCK_FLAG; // Protocol Control Byte
  tSCT->cTPDU[CCID_TPDU_LENGTH] = 0;

  tSCT->cTPDULength = CCID_TPDU_OVERHEAD; // the length of the TPDU
  tSCT->cTPDUSequence++;                  // switch sequence

  tSCT->cTPDU[CCID_TPDU_OVERHEAD - 1] =
      GenerateCRC((unsigned char *)&tSCT->cTPDU, CCID_TPDU_PROLOG);
}
...
unsigned short SendTPDU(typeSmartcardTransfer *tSCT) {
  ...
  CRD_SendCommand((unsigned char *)tSCT->cTPDU, tSCT->cTPDULength, CCID_TRANSFER_BUFFER_MAX,
  ...
  memcpy(&tSCT->cAPDU[tSCT->cAPDUAnswerLength], &tSCT->cTPDU[CCID_TPDU_DATASTART],
...
unsigned short CcidSelectOpenPGPApp(void) {
  unsigned short cRet;
  unsigned char cOpenPGPApp[CCID_SIZE_OPEN_PGP_APP] = {0x00, 0xA4, 0x04, 0x00, 0x06, 0xD2,
                                                       0x76, 0x00, 0x01, 0x24, 0x01};
...
unsigned short CcidDecipher(unsigned char *nRetSize) {
  ...
  unsigned char cDecipherString[CCID_DECIPHER_STRING_SIZE] = {
      };
  ...
  memcpy(&tSCT.cAPDU[CCID_LC + 2], cDecipherString,
         CCID_DECIPHER_STRING_SIZE); // string to decipher
...
int AccessTestGPG(void) {
  ...
  /* GETDATA 0x5F 0x52 - Historical bytes, up to 15 bytes (dec.), according to ISO 7816-4. Card
   * capabilities shall be included.*/
...
uint8_t getByteOfData(uint8_t x) { return tSCT.cAPDU[x]; }
