...
void CCID_BulkOutMessage(void) {
  ...
  case RECEIVE_UNCORRECTLENGTH_END:
    ...
    if (UsbDataLength != 0xFF) {
      ...
      if ((UsbMessageLength > 0) &&
          (BulkStatus == RECEIVE_OTHER_END)) { // Jeffery Lee 2002/12/05 <+> <1>
...
unsigned char CCID_BulkInMessage(void) {
  ...
  case TRANSMIT_HEADER:
    UsbMessageLength = USB_MESSAGE_HEADER_SIZE + MAKEWORD(UsbMessageBuffer[OFFSET_DWLENGTH + 1],
  ...
  if (bSlotChangedFlag) {
    ...
    if (SlotStabilizationDelay == 0x00) {
      if ((UsbIntMessageBuffer[OFFSET_INT_BMSLOTICCSTATE] == 0x02) && bPreviousSlotStateFlag) {
        ...
        // needed ??				if( USB_SendDataEP1(UsbIntMessageBuffer, MessageSize) ==
        // REQ_SUCCESS )
        {
...
void CCID_SetCardState(unsigned char nState) { cCRD_CardPresent = nState; }
...
unsigned char CCID_GetCardState(void) { return (cCRD_CardPresent); }
