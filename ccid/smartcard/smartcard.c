...
static void RCC_Configuration(void) {
  ...
  if (HSEStartUpStatus == SUCCESS) {
    ...
    RCC_PLLConfig(RCC_PLLSource_HSE_Div1, RCC_PLLMul_9); // RB RCC_PLLMul_9);
...
void SC_PowerCmd(FunctionalState NewState) {
  if (NewState != DISABLE) {
    ...
    USART_SmartCardCmd(USART3, DISABLE); // card is always on
    GPIO_ResetBits(SMARTCARD_POWER_PORT, SMARTCARD_POWER_PIN_1);
    GPIO_ResetBits(SMARTCARD_POWER_PORT, SMARTCARD_POWER_PIN_2);
  }
}
...
* Return         : None
*******************************************************************************/
void SC_Reset(BitAction ResetState) { GPIO_WriteBit(GPIO_RESET, SC_RESET, ResetState); }
...
void SC_SetHwParams(u8 cBaudrateIndex, u8 cConversion, u8 Guardtime, u8 Waitingtime) {
  ...
  USART_SetGuardTime(USART3, 12 + Guardtime);
}
...
static s8 CheckForSCStatus(SC_ADPU_Commands *SC_ADPU, SC_ADPU_Responce *SC_ResponceStatus) {
  ...
  if (((locData & (u8)0xFE) == (((u8) ~(SC_ADPU->Header.INS)) & (u8)0xFE)) ||
...
void SC_Init(void) {
  ...
  */
  ...
  USART_SetPrescaler(USART3, 0x05); // RB0x05

  /* USART Guard Time set to 16 Bit */
  USART_SetGuardTime(USART3, 1); // RB 16
...
int CRD_SendCommand(unsigned char *pTransmitBuffer, unsigned int nCommandSize,
...
// when INS 0x20 is send, after 4 byte the germalto	card (only?) send a ACK byte
#ifdef GERMALTO_CARD
  ...
  for (i = 0; i < ICC_MESSAGE_BUFFER_MAX_LENGTH - USB_MESSAGE_HEADER_SIZE;
       i++) // max buffer size (had to be checked)
  {
    ...
    case 0:
      ...
      // Get Error state
      if (((pTransmitBuffer[i] & (u8)0xF0) ==
           0x60)) // ||  ((pTransmitBuffer[i] & (u8)0xF0) == 0x90))
      {
...
int scTest(void) {
  ...
  /*
  {
    ...
    unsigned int   nReceivedAnswerSize;
                  ...
                  pTransmitBuffer[0] = 0xFF;
                  pTransmitBuffer[1] = 0x11;
                  pTransmitBuffer[2] = 0x13;
                  pTransmitBuffer[3] = 0xFD;
                  CRD_SendCommand (pTransmitBuffer,4,2,&nReceivedAnswerSize);
