...
static void SDIO_SetParam(SDIO_InitTypeDef *SDIO_InitStruct, uint32_t ClockDiv, uint32_t BusWide);
SD_Error SD_ReadBlock_org(uint32_t addr, uint32_t *readbuff, uint16_t BlockSize);
SD_Error SD_WriteBlock_org(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize);

/* Private functions ---------------------------------------------------------*/

void SDIO_SetParam(SDIO_InitTypeDef *SDIO_InitStruct, uint32_t ClockDiv, uint32_t BusWide) {
  SDIO_InitStructure.SDIO_ClockDiv = ClockDiv;
  SDIO_InitStructure.SDIO_ClockEdge =
      SDIO_ClockEdge_Rising; // SDIO_ClockEdge_Falling; //SDIO_ClockEdge_Rising;
  SDIO_InitStructure.SDIO_ClockBypass =
      SDIO_ClockBypass_Disable; // SDIO_ClockBypass_Enable;//  SDIO_ClockBypass_Disable;
...
SD_Error SD_Init(void) {
  ...
  errorstatus = SD_PowerON(); // needed for correct start up
  ...
  /* Set SD interface parameter */
  /*
    SDIO_InitStructure.SDIO_ClockDiv            = SDIO_TRANSFER_CLK_DIV;
    SDIO_InitStructure.SDIO_ClockEdge           = SDIO_ClockEdge_Falling; //SDIO_ClockEdge_Rising;
    ...
    SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
  */
  SDIO_SetParam(&SDIO_InitStructure, SDIO_TRANSFER_CLK_DIV, SDIO_BusWide_1b);
...
SD_Error SD_PowerON(void) {
  ...
  /* Configure the SDIO peripheral */
  /*
    SDIO_InitStructure.SDIO_ClockDiv            = SDIO_INIT_CLK_DIV; // HCLK = 72MHz, SDIOCLK =
    72MHz, SDIO_CK = HCLK/(178 + 2) = 400 KHz
    ...
    SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
  */
  SDIO_SetParam(&SDIO_InitStructure, SDIO_INIT_CLK_DIV, SDIO_BusWide_1b);
  SDIO_Init(&SDIO_InitStructure);
    ...
    if (SDIO_BusWide_8b == WideMode) {
      ...
      if (SD_OK == errorstatus) {
        ...
        /*        SDIO_InitStructure.SDIO_ClockDiv            = SDIO_TRANSFER_CLK_DIV;
                ...
                SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
        */
        SDIO_SetParam(&SDIO_InitStructure, SDIO_TRANSFER_CLK_DIV, SDIO_BusWide_4b);
...
SD_Error SD_ReadBlock_org(uint32_t addr, uint32_t *readbuff, uint16_t BlockSize) {
  ...
  if (DeviceMode == SD_POLLING_MODE) {
    ...
    if (SDIO_GetFlagStatus(SDIO_FLAG_DTIMEOUT) != RESET) {
      ...
      SDIO_StartSDIOReadWait(DISABLE);
      return (errorstatus);
    ...
    SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR |
...
/*******************************************************************************
  ...
  SD_ReadBlock
        ...
        Wrapper for correct datatransfer

*******************************************************************************/

SD_Error SD_ReadBlock(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize) {
  SD_Error errorstatus;
  int i, i1;

  for (i = 0; i < 100; i++) {
    errorstatus = SD_ReadBlock_org(addr, (uint32_t *)writebuff, BlockSize);
    if (SD_OK == errorstatus) {
      break;
    }
    for (i1 = 0; i1 < 1000; i1++)
...
* Return         : SD_Error: SD Card Error code.
*******************************************************************************/
SD_Error SD_ReadMultiBlocks(uint32_t addr, uint32_t *readbuff, uint16_t BlockSize,
  ...
  if (NumberOfBlocks > 1) {
    ...
    if (DeviceMode == SD_POLLING_MODE) {
      /* Polling mode */
      while (!(SDIO->STA & (SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DATAEND |
...
SD_Error SD_WriteBlock_org(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize) {
  ...
  if (DeviceMode == SD_POLLING_MODE) {
    while (!(SDIO->STA & (SDIO_FLAG_DBCKEND | SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL |
                          SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR))) {
    ...
    SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_FLAG_TXFIFOHE |
    ...
    SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_TXUNDERR |
                      SDIO_IT_STBITERR,
                  ENABLE);
...
SD_Error SD_WriteBlock(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize) {
  ...
  for (i = 0; i < 1000; i++) {
    errorstatus = SD_WriteBlock_org(addr, writebuff, BlockSize);
    if (SD_OK == errorstatus) {
      break;
    }
  }
...
* Return         : SD_Error: SD Card Error code.
*******************************************************************************/
SD_Error SD_WriteMultiBlocks(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize,
                             uint32_t NumberOfBlocks) {
  SD_Error errorstatus = SD_OK;
  uint8_t power = 0, cardstate = 0;
  ...
  if (NumberOfBlocks > 1) {
    ...
    if (DeviceMode == SD_POLLING_MODE) {
      while (!(SDIO->STA & (SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DATAEND |
                            SDIO_FLAG_DTIMEOUT | SDIO_FLAG_STBITERR))) {
        if (SDIO_GetFlagStatus(SDIO_FLAG_TXFIFOHE) != RESET) {
...
SD_Error SD_SendSDStatus(uint32_t *psdstatus) {
  ...
  for (count = 0; count < 16; count++) {
    psdstatus[count] =
        ((psdstatus[count] & SD_0TO7BITS) << 24) | ((psdstatus[count] & SD_8TO15BITS) << 8) |
...
static SD_Error CmdResp6Error(uint8_t cmd, uint16_t *prca) {
  ...
  if (SD_ALLZERO ==
      (response_r1 & (SD_R6_GENERAL_UNKNOWN_ERROR | SD_R6_ILLEGAL_CMD | SD_R6_COM_CRC_FAILED))) {
...
* Return         : None
*******************************************************************************/
static uint8_t convert_from_bytes_to_power_of_two(uint16_t NumberOfBytes_l) {
  uint8_t count = 0;

  while (NumberOfBytes_l != 1) {
    NumberOfBytes_l >>= 1;
    count++;
  }
  return (count);
