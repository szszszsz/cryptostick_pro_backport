...
// Get root dir
	pBS = (tBootSectorType *)pRamDisk;
	nRootDirOffset = pBS->BytesPerSector * pBS->SectorsPerCluster * (pBS->FATSectors * pBS->FATCount + 1);		// + 1 = + boot sector 
...
// Set mem to space
	nDataSecOffset = nRootDirOffset + pBS->RootEntries * sizeof(tDirEntryType); // should be nRootDirOffset + 0x200
